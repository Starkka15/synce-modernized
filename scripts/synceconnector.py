...
# defaults
config_file = '/usr/local/etc/synce-connector.conf'
...
def rndis_static_config(iface):
    ...
    cmd_list = ["/usr/sbin/ifconfig",iface,DEFAULT_RNDIS_LOCAL_IP,"netmask",DEFAULT_RNDIS_NETMASK,"broadcast",DEFAULT_RNDIS_BROADCAST]
...
def rndis_dhcp_config(iface):
    ...
    pidfile = "/usr/local/var/run/dhclient-synce-"+iface+".pid"
    leasefile = "/usr/local/var/run/dhclient-synce-"+iface+".lease"
    configfile = "/usr/local/share/synce-core/dhclient.conf"

    cmd_list = ["/usr/sbin/dhclient","-pf",pidfile,"-lf",leasefile,"-cf",configfile,iface]
...
def rndis_dhcp_unconfig(iface):
    ...
    cmd_list = ["/usr/sbin/dhclient","-r","-pf",pidfile,"-lf",leasefile,"-cf",configfile,iface]
...
def serial_start_device(device_file, local_ip, remote_ip, linkname):
    ...
    cmd_list = [PPPD, device_file, DEVSPEED, "connect", "/usr/local/libexec/synce-serial-chat", ipaddr, "ms-dns", local_ip]
    for item in pppopts:
...
def run_dccm(device_path, device_ip, local_ip, rndis):
    ...
    cmd_list = ["/usr/local/libexec/dccm", "--device-path="+device_path, "--device-ip="+device_ip, "--local-ip="+local_ip, "--log-level="+dccm_log_level]
    if rndis == True:
